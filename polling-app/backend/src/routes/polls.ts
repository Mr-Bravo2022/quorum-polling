...
import { aggregate, initTally, restoreTally, getTally, clearTally, VoteMessage } from '../patterns/aggregator';
...
router.post('/polls', async (ctx) => {
  ...
  metrics.incr('polls_created');
  log.info('poll.created', { pollId: id, question, options: options.length });
...
router.post('/polls/:id/vote', async (ctx) => {
  ...
  if (!actor || actor.getSnapshot().value !== 'open') {
    ...
    log.warn('vote.rejected', { pollId, optionIndex, reason: 'poll-not-open' });
    ctx.status = 409;
  ...
  metrics.recordVote(Date.now() - t0);   // Rate + Duration
  log.info('vote.cast', { pollId, optionIndex, total: tally?.total ?? 0 });
  ctx.body = tally;
});
...
// DELETE /api/polls/:id — permanently remove a poll and its audit rows.
// Owner-initiated cleanup from the profile. Idempotent: deleting a poll that's
// already gone still succeeds (204), so the UI can clear stale "unavailable" rows.
router.delete('/polls/:id', async (ctx) => {
  const pollId = ctx.params.id;
  const db = getDb();

  db.run('DELETE FROM votes WHERE poll_id = ?', [pollId]);
  db.run('DELETE FROM poll_status_log WHERE poll_id = ?', [pollId]);
  db.run('DELETE FROM polls WHERE id = ?', [pollId]);
  persist();

  // Drop the in-memory state-chart actor + tally.
  pollActors.get(pollId)?.stop();
  pollActors.delete(pollId);
  clearTally(pollId);

  metrics.incr('polls_deleted');
  log.info('poll.deleted', { pollId });
  ctx.status = 204;
});

// GET /api/metrics — RED-method metrics view (Observability: metrics half).
router.get('/metrics', async (ctx) => {
  // Active polls is a live gauge: state-chart actors currently in `open`.
  const activePolls = [...pollActors.values()]
    .filter((a) => a.getSnapshot().value === 'open').length;
  ctx.body = metrics.snapshot(activePolls);
});

// POST /api/metrics/event — record a client-side event (join success / camera
// fallback), so the RED view includes the browser-side join story.
router.post('/metrics/event', async (ctx) => {
  const { type } = ctx.request.body as { type?: string };
  if (type === 'join_success' || type === 'camera_fallback') {
    metrics.recordClientEvent(type);
    ctx.status = 204;
  } else {
    ctx.status = 400;
    ctx.body = { error: 'unknown metric event' };
  }
});
...
async function transition(ctx: any, event: PollEvent['type']): Promise<boolean> {
  ...
  if (status === before) {
    ...
    log.warn('poll.transition_rejected', { pollId, event, from: before });
    ctx.status = 409;
  ...
  if (status === 'closed') metrics.incr('polls_closed');
  log.info('poll.status_changed', { pollId, from: before, to: status });
  ctx.body = { status };
  return true;
}
...
export function rehydrate(): void {
  ...
  log.info('poll.rehydrated', { count: res[0].values.length });
}
